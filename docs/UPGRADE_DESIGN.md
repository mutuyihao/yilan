...
- 侧栏诊断、阅读快照、历史项和文章 meta/trust card 的展示数据已拆到 `shared/diagnostics-view.js`、`shared/reader-view.js`、`shared/history-view.js`、`shared/sidebar-meta-view.js`；后续仍需继续压缩 `sidebar.js` 的运行编排和 DOM 绑定职责。
...
4. 下一步优先用 `shared/transport-utils.js` 替换 `background.js` 的重复 SSE/raw body 解析函数。
5. 后续再把历史面板运行逻辑从 `sidebar.js` 继续迁出；当前已先抽出 `shared/history-view.js` 作为低风险试点。
