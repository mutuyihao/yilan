...
- `background.js` 已复用 `shared/transport-utils.js` 的 SSE / raw body 解析逻辑，避免后台重复维护 transport 细节；active runs、port runs 和取消控制已抽到 `background/run-state.js`，阅读页临时会话已抽到 `background/reader-sessions.js`。
...
5. 已把历史面板运行逻辑从 `sidebar.js` 迁出到 `sidebar/history.js`，并已在 Phase 3 把后台 run state 抽到 `background/run-state.js`，reader session 抽到 `background/reader-sessions.js`；后续优先继续抽 `background/entrypoints.js`。
