...
TypeScript、构建链和 React 的专项迁移计划不在本文展开，见 [TS + React 迁移评估与执行计划](TS_REACT_MIGRATION.md)。
...
- `sidebar.js` 当前约 32KB，已经从早期单体中拆出多个控制器，但仍承担侧栏初始化、依赖注入、状态编排和跨模块协调。
- `background.js` 当前约 38KB，已拆出入口、运行状态和阅读会话模块，但仍承担连接测试、模型列表刷新、自动 endpoint 试探、请求执行、重试和错误归一。
- `popup.js` 当前约 50KB，混合了设置表单、厂商 preset、配置方案、模型列表刷新、自动保存、入口检查、连接测试和诊断展示。
- `style.css` 当前约 38KB，仍是侧栏基础样式入口；新增高级视觉样式已拆到 `sidebar-premium.css`，但后续资料库视图仍可能继续膨胀。
...
- 侧栏诊断、阅读快照、历史项和文章 meta/trust card 的展示数据已拆到 `shared/diagnostics-view.js`、`shared/reader-view.js`、`shared/history-view.js`、`shared/sidebar-meta-view.js`；状态对象、导航策略常量和 DOM 元素绑定已迁出到 `sidebar/state.js`；历史面板运行逻辑已迁出到 `sidebar/history.js`，Markdown 导出和分享卡已迁出到 `sidebar/export.js`，阅读页打开已迁出到 `sidebar/reader-session.js`，主摘要、二次生成、取消和流式连接已迁出到 `sidebar/generation.js`，摘要模式控件已迁出到 `sidebar/mode-control.js`，侧栏摘要渲染、状态提示、来源/trust card DOM 写入和诊断展示已迁出到 `sidebar/render.js`，事件绑定已迁出到 `sidebar/events.js`；`sidebar.js` 后续主要保留编排职责。
- `sidebar/render.js` 与 `reader.js` 仍各自负责页面内 Markdown 渲染/净化和 DOM 写入，这部分暂不抽成共享渲染器，避免把页面差异和安全边界混在一个工具里。
- `background.js` 已复用 `shared/transport-utils.js` 的 SSE / raw body 解析逻辑，避免后台重复维护 transport 细节；右键菜单、快捷键和入口状态已抽到 `background/entrypoints.js`，active runs、port runs 和取消控制已抽到 `background/run-state.js`，阅读页临时会话已抽到 `background/reader-sessions.js`。连接测试、模型列表刷新、自动 endpoint 缓存和 `/v1` 修正仍留在后台主编排中。
- TypeScript 基础门禁已落地为 `npm run typecheck`，并用 `types/` 中的消息、历史、设置和诊断契约先锁住后续迁移边界。
...
- `sidebar/state.js`：集中状态对象、运行状态、当前文章、可见记录、导航策略常量和 DOM 元素绑定。已完成。
- `sidebar/render.js`：来源卡片、信任卡片、摘要渲染、错误态、进度态。已完成。
- `sidebar/history.js`：历史搜索、站点分组、收藏、删除、记录绑定。已完成。
- `sidebar/generation.js`：主摘要、分段摘要、二次生成、取消、流式连接。已完成。
- `sidebar/export.js`：Markdown 导出、分享图。已完成。
- `sidebar/reader-session.js`：阅读页快照和打开逻辑。已完成。
- `sidebar/mode-control.js`：摘要模式选项、菜单开关、键盘交互和原生 select 同步。已完成。
- `sidebar/events.js`：DOM 事件绑定和初始化编排。已完成。
...
- 已把运行状态、active runs、port runs、取消控制抽成 `background/run-state.js`。
- 已把右键菜单、快捷键、入口状态抽成 `background/entrypoints.js`。
- 已把阅读会话清理和创建抽成 `background/reader-sessions.js`。
- 模型列表刷新、OpenAI 兼容接口的 `endpointMode=auto` 试探和 `/v1` 自动修正仍是后台编排能力，暂不继续拆散，避免把 provider 兼容策略和请求运行路径拆成过细的隐式依赖。
...
5. 已把状态对象、导航策略常量和 DOM 元素绑定从 `sidebar.js` 迁出到 `sidebar/state.js`，历史面板运行逻辑迁出到 `sidebar/history.js`，Markdown 导出和分享卡迁出到 `sidebar/export.js`，阅读页打开迁出到 `sidebar/reader-session.js`，主摘要、二次生成、取消和流式连接迁出到 `sidebar/generation.js`，摘要模式控件迁出到 `sidebar/mode-control.js`，摘要渲染、来源/trust card、状态提示和诊断展示迁出到 `sidebar/render.js`，事件绑定迁出到 `sidebar/events.js`，并已在 Phase 3 把后台 entrypoints、run state 和 reader session 分别抽到 `background/entrypoints.js`、`background/run-state.js`、`background/reader-sessions.js`。
6. 已新增 TypeScript typecheck 和核心契约类型；后续再推进构建链和 React，不在当前批次改运行产物。

这些步骤的特点是收益明确、风险可控、功能感知小，适合作为技术债治理的起手式。
...
构建链、TypeScript 和 React 已拆到独立迁移设计中，必须证明能降低维护成本，并通过测试门禁后再逐步落地。
