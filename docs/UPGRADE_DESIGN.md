...
- `background.js` 已复用 `shared/transport-utils.js` 的 SSE / raw body 解析逻辑，避免后台重复维护 transport 细节；右键菜单、快捷键和入口状态已抽到 `background/entrypoints.js`，active runs、port runs 和取消控制已抽到 `background/run-state.js`，阅读页临时会话已抽到 `background/reader-sessions.js`。
...
- `sidebar/export.js`：Markdown 导出、分享图。
...
6. 已新增 TypeScript typecheck 和核心契约类型；后续再推进构建链和 Preact，不在当前批次改运行产物。
