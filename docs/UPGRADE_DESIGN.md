...
- 侧栏诊断、阅读快照、历史项和文章 meta/trust card 的展示数据已拆到 `shared/diagnostics-view.js`、`shared/reader-view.js`、`shared/history-view.js`、`shared/sidebar-meta-view.js`；历史面板运行逻辑已迁出到 `sidebar/history.js`，Markdown 导出和分享卡已迁出到 `sidebar/export.js`，阅读页打开已迁出到 `sidebar/reader-session.js`，主摘要、二次生成、取消和流式连接已迁出到 `sidebar/generation.js`，摘要模式控件已迁出到 `sidebar/mode-control.js`，事件绑定已迁出到 `sidebar/events.js`；后续仍需继续压缩 `sidebar.js` 的 DOM 绑定职责。
