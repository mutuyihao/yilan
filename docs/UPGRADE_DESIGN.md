...
5. 已把历史面板运行逻辑从 `sidebar.js` 迁出到 `sidebar/history.js`，并已在 Phase 3 把后台 entrypoints、run state 和 reader session 分别抽到 `background/entrypoints.js`、`background/run-state.js`、`background/reader-sessions.js`。
