...
- 侧栏诊断、阅读快照、历史项和文章 meta/trust card 的展示数据已拆到 `shared/diagnostics-view.js`、`shared/reader-view.js`、`shared/history-view.js`、`shared/sidebar-meta-view.js`；历史面板运行逻辑已迁出到 `sidebar/history.js`，Markdown 导出和分享卡已迁出到 `sidebar/export.js`；后续仍需继续压缩 `sidebar.js` 的生成、阅读会话和 DOM 绑定职责。
...
5. 已把历史面板运行逻辑从 `sidebar.js` 迁出到 `sidebar/history.js`，Markdown 导出和分享卡迁出到 `sidebar/export.js`，并已在 Phase 3 把后台 entrypoints、run state 和 reader session 分别抽到 `background/entrypoints.js`、`background/run-state.js`、`background/reader-sessions.js`。
